...
def laplace(r):
    ...
    laplace_matice = np.diag(off_diag,k=1)+np.diag(off_diag,k=-1)+np.diag(main_diag)
    return laplace_matice

def spinorbit(r,R,a,V0,r_0):
    so = np.zeros(Number)
    for i in range(len(r)):
        so[i]=-0.44*V0*(r_0/hbar)*(r_0/hbar)*(1/r[i])*np.exp((r[i]-R)/a)/(a*(np.exp((r[i]-R)/a)+1)**2)
    return so
...
def create_ham_proton(j,l,r,R,a,V0proton,r_0,Z,m_proton):    
    ...
    hamiltonian_proton = (-hbarc**2/(2.0*m_proton))*(laplace_term - angular_matice) + (woodsaxon_term_proton+columbvmev_matice+scalarLS_term_proton)
...
def plot_potencial(j,l,r,R,a,V0neutron,r_0,V0proton):
    ...
    ax[0].plot(r,woodsaxon(r,R,a,V0proton), color='y', label="Wood-Saxon potencial")
    ax[0].plot(r,scalarLS(j,l,r,R,a,V0proton,r_0), color='r', label="Spin-Orbit")
    ax[0].plot(r,columb(r,Z,R), color='b', label="Coulomb potencial")
    ax[0].plot(r,columb(r,Z,R) + woodsaxon(r,R,a,V0proton) + scalarLS(j,l,r,R,a,V0proton,r_0), color='g', label="Wood-Saxon + Coulomb potencial + Spin-Orbit")
    ax[0].set_xlabel("r[fm]")
    ax[0].set_ylabel("V[MeV]")
    ax[0].legend(loc=0)
    ax[0].set_title("Proton potencial V(r), l=%s"%l + " j=%s"%j)

    ax[1].plot(r,woodsaxon(r,R,a,V0neutron), color='y', label="Wood-Saxon potencial")
    ax[1].plot(r,scalarLS(j,l,r,R,a,V0neutron,r_0), color='r', label="Spin-orbit")
    ax[1].plot(r,woodsaxon(r,R,a,V0neutron) + scalarLS(j,l,r,R,a,V0neutron,r_0), color='g', label="Wood-Saxon + Spin-Orbit")
    ax[1].set_ylim([-60,50])
    ax[1].set_xlabel("r[fm]")
    ax[1].set_ylabel("V[MeV]")
    ax[1].set_title("Neutron potencial V(r),l=%s"%l + " j=%s"%j)
    ax[1].legend(loc=0)
...
for i in range(len(l1)):
    ...
    print('----------------------------------------------------------------------------------------')
    print('Orbital angular momentum l: %s'%l1[i] + '\nTotal angular momentum j: %s \n'%j1[i])

    hamiltonian_proton = create_ham_proton(j1[i],l1[i],r,R,a,V0proton,r_0,Z,m_proton)
    hamiltonian_neutron = create_ham_neutron(j1[i],l1[i],r,R,a,V0neutron,r_0,m_neutron)
    ...
    print( 'E_0proton = %s '%eigenvalues_proton[0] + ' E_0neutron = %s \n'%eigenvalues_neutron[0]) 

    densities = [np.absolute(eigenvectors_proton[i, :])**2 for i in range(len(eigenvalues_proton))]
...
plot_densities(r,densities_proton,densities_neutron,energies_proton,energies_neutron,l,j)
